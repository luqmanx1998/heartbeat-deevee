  ...
  return (
    <main className="min-h-screen bg-[#090909] p-8 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <h1 className="mt-4 text-4xl font-semibold">Metrics</h1>

        <div className="mt-8 grid gap-4 md:grid-cols-4">
          <MetricCard title="Homepage views" value={homepageViews.length} />
          <MetricCard title="Store views" value={storeViews.length} />
          <MetricCard title="Unique visitors" value={uniqueVisitors} />
          <MetricCard title="Total views" value={views.length} />
        </div>
        ...
        <section className="mt-10 rounded-2xl border border-white/10 bg-white/[0.04] p-6">
          ...
          <div className="mt-5 space-y-3">
            {sourceRows.length === 0 ? (
              <p className="text-white/50">No source data yet.</p>
            ) : (
              ...
              <tbody>
                {views.slice(0, 30).map((view) => (
                  <tr key={view.id} className="border-t border-white/10">
                    ...
                    <td className="py-3">
                      {view.utm_source || "direct / unknown"}
                    </td>
